...
def onboarding(token, guild_id):
    t = token.split(".")[0]
    current_time = datetime.datetime.now().strftime("%H:%M:%S")
    try:
        url = f"https://discord.com/api/v9/guilds/{guild_id}/onboarding"
        req = requests.get(url, headers=headers(token))
        ...
        for prompt in data["prompts"]:
            ...
            for option in prompt["options"]:
                if option:
                    ...
                    print("No onboarding in this server.")
                    return
        try:
            json_data = {
            ...
            url2 = f"https://discord.com/api/v9/guilds/{guild_id}/onboarding-responses"

            response = requests.post(url2 ,headers=headers(token), json=json_data)

            match req.status_code:
                case 200:
                    if response.status_code == 200:
                        print(f"{W}{current_time}{C}||Onboarding bypass succesfully: {V}{response.status_code}")
                    else:
                        print(response.text)
                        print(f"{W}{current_time}{C}||{I}ERROR: {response.status_code}")
                case 401:
                    print(response.text)
                    print(f"{W}{current_time}{C}||{I}DEAD TOKEN: {response.status_code}")
                case _:
                    print(response.text)
                    print(f"{W}{current_time}{C}||{I}ERROR: {response.status_code}")
                     
        except Exception as e:
            print(f"{W}{current_time}{C}||{I}FAILED: {response.status_code}")
