...
def send_message(token, content, url, proxies, stop_event):
    url = f"https://discord.com/api/v9/channels/{url}/messages"

    headers_data = headers(token)
    ...
    with requests.Session() as session:
        while not stop_event.is_set():
            ...
            try:
                response = session.post(url, headers=headers_data, json=data, proxies=proxies, timeout=5)
                if response.status_code == 200:
                    print(f"{W}{current_time}{C}||Message sent successfully: {V}{response.status_code}")
                elif response.status_code == 429:
                    print(f"{W}{current_time}{C}||{I}Rate-limited. Waiting...")
                ...
                print(f"{W}{current_time}{C}||{I}Invalid proxy")