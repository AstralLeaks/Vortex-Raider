...
def accept_rules(token, values, server_id):
    url = f"https://discord.com/api/v9/guilds/{server_id}/requests/@me"
    ...
    response = requests.put(url, headers=head, params=None, json=body)
    current_time = datetime.datetime.now().strftime("%H:%M:%S")
    print(f"{W}{current_time}{C}||{V}Successfully accepted server-rules {response.status_code}")