...
def send_message_massping(tokens, content, channel_id, proxies, stop_event, user_id, random_chars):
    while not stop_event.is_set():
        headers_data = headers(tokens)
        url = f"https://discord.com/api/v9/channels/{channel_id}/messages"
        
        data = {
            "content": content + f'<@{user_id}>' + " | " + random_chars,
        }

        with requests.Session() as session:
            while not stop_event.is_set():
                current_time = datetime.datetime.now().strftime("%H:%M:%S")
                response = session.post(url, headers=headers_data, json=data, proxies=proxies)
