...
def change_nick(token, nick, guild_id):
    url = f"https://discord.com/api/v9/guilds/{guild_id}/members/%40me/nick"

    data = "{\"nick\":\"" + nick + "\"}"

    response = requests.patch(url, headers=headers(token), data=data)
    if response.json().get("nick") == nick:
        current_time = datetime.datetime.now().strftime("%H:%M:%S")
        print(f"{W}{current_time}{C}||{V}Successfully changed nickname")
    else:
        current_time = datetime.datetime.now().strftime("%H:%M:%S")
        print(f"{W}{current_time}{C}||{I}Failed to change nickname")